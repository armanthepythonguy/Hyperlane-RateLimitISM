...
class LiquidityLayerApp extends HyperlaneApp_1.HyperlaneApp {
    ...
    async fetchCircleMessageTransactions(chain) {
        ...
        const req = await (0, fetch_1.fetchWithTimeout)(url);
    ...
    async attemptPortalTransferCompletion(message) {
        ...
        if (!(0, utils_1.eqAddress)(transferTokenAddress, ethers_1.ethers.constants.AddressZero)) {
        ...
        const emitter = (0, utils_1.strip0x)((0, utils_1.addressToBytes32)(this.config[message.origin].portal.portalBridgeAddress));
        const vaa = await (0, fetch_1.fetchWithTimeout)(`${PORTAL_VAA_SERVICE_TESTNET_BASE_URL}${wormholeOriginDomain}/${emitter}/${message.portalSequence}`).then((response) => response.json());
        ...
        try {
            await this.multiProvider.handleTx(message.destination, destinationPortalAdapter.completeTransfer((0, utils_1.ensure0x)(Buffer.from(vaa.vaaBytes, 'base64').toString('hex'))));
        }
        catch (error) {
    ...
    async attemptCircleAttestationSubmission(message) {
        ...
        const attestationsB = await (0, fetch_1.fetchWithTimeout)(`${baseurl}${messageHash}`);
